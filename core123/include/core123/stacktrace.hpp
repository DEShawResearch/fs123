...
// stacktrace_error is a runtime_error whose 'what' message contains a
// stack trace at the point at which the object was created.  It uses
// Francois-Xavier Bourlet's backward-cpp library (MIT License) to
// do the "heavy lifting".  Usage:
//
//   if(problem)
...
//   }
//
// To associate a stack trace with a specific kind of error (which
// might match a catch expression higher up the stack), use throw_nest
// in exnest.hpp:
//
//   if(problem)
...
struct stacktrace_error : public std::runtime_error{
    ...
    explicit stacktrace_error(const std::string& whatarg, size_t depth=100) : std::runtime_error(whatarg.c_str()){}
    ...
    virtual const char* what() const noexcept try {
        if(!msg_)
            msg_ = std::make_shared<std::string>(std::runtime_error::what() + std::string(": ") + trace(*st_));
        return msg_->c_str();
    }catch(...){
    ...
    static std::string
    trace(const backward::StackTrace& st) {
        ...
        for (size_t i = 0; i < st.size(); ++i) {
            ...
            trace_oneline(i, trace.source, trace, oss);
            for (const auto& v : trace.inliners)
                trace_oneline(i, v, trace, oss);
        }
        return oss.str();
    }
